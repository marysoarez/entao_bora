...
class LocationDatasourceImpl implements ILocationDatasource {
  ...
  Future<List<AddressDto>> searchAddress(String query) async {
    ...
    for (final item in suggestions) {
      ...
      final address = await _getPlaceDetails(placeId);
  ...
  Future<AddressDto?> _getPlaceDetails(String placeId) async {
    ...
    final displayName =
        (data['displayName'] as Map<String, dynamic>?)?['text']?.toString() ??
  ...
  Future<AddressDto?> geocodeAddress(AddressDto address) async {
    final parts = [
      if (address.street?.trim().isNotEmpty ?? false) address.street!.trim(),
      if (address.number?.trim().isNotEmpty ?? false) address.number!.trim(),
      if (address.neighborhood?.trim().isNotEmpty ?? false)
        address.neighborhood!.trim(),
      if (address.city?.trim().isNotEmpty ?? false) address.city!.trim(),
      if (address.state?.trim().isNotEmpty ?? false) address.state!.trim(),
    ...
    return AddressDto(
      displayName:
          result['formatted_address']?.toString() ?? address.displayName,
  ...
  Future<AddressDto?> reverseGeocode(LocationEntity location) async {
    ...
    return AddressDto(
      ...
      postalCode: getComponent('postal_code'),
      location: LocationEntity(
        latitude: (locationData['lat'] as num).toDouble(),
